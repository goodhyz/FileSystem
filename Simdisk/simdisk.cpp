...
int main() {
    ...
    while (1) {
        ...
        if (shm->is_login_prompt) {
            ...
            User user = shm->user_list[std::stoi(user_index)];
            // 判断账号密码正确性
            ...
            if (shm->is_login_success) {
                shm->user_list[std::stoi(user_index)] = user;
        ...
        if (shm->ready) {
            // 确定指令的发起用户及其所在目录
            User user = shm->user_list[shm->cur_user];
            Inode cur_inode = Inode::read_inode(shm->cur_dir_inode_id);
            ...
            if (cmd == "exit" || cmd == "EXIT") {
                if (options.find("-h") != options.end()) {
                    ...
                    shm->user_list[shm->cur_user] = User();
                    shm->done = true;
                    shm->ready = false;
                    break;
                }
                    ...
                    while (1) {
                        ...
                        if (!is_file_exit(file_name, Inode::read_inode(start_id))) {
                            ...
                            if (options["-i"].empty()) {
                                ...
                                if (!is_able_to_write(get_file_inode_id(file_name, cur_inode), user)) {
                ...
                if (options.find("-h") != options.end()) {
                    ...
                    if (!is_able_to_read(cur_inode.i_id, user)) {
                        ...
                        shell_output = show_directory(cur_inode.i_id, false);
                    } else {
                        shell_output = show_directory(cur_inode.i_id, true);
                    }
                }
            ...
            strncpy(shm->result, shell_output.c_str(), sizeof(shm->result) - 1);
            shm->cur_dir_inode_id = cur_inode.i_id;
            shm->done = true;
            shm->ready = false;
        }
        Sleep(10); // 避免无用的循环
